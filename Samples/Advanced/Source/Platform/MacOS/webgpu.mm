 ...
 * the \c #swapPref.
 */
static void initSwapChain(WGPUBackendType backend, WGPUDevice device, NSWindow* window) {
	switch (backend) {
	case WGPUBackendType_Metal:
		if (impl::swapImpl.userData == nullptr) {
			impl::swapImpl = createMetalSwapChain(window);
...
WGPUDevice webgpu::create(NSWindow* window, WGPUBackendType type) {
